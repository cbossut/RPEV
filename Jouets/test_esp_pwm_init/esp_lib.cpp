...
void initPins(const byte nbBtns, const byte* btnPins, const uint16_t btnInit, const byte PWMPin, int PWMInit) {
  byte i;
  for (i = 0 ; i < nbBtns ; i++) {
    digitalWrite(btnPins[i], bitRead(btnInit, i));
  }
   EEPROM.begin(4);
 PWMInit =  EEPROM.read(1) + (EEPROM.read(2) << 8);
  analogWrite(PWMPin, PWMInit);
  btnStates = btnInit;
  PWMState = PWMInit;
  Serial.print("PWM inital :  ");
  Serial.println(PWMInit);
}
...
//    char buf[30];
//    sprintf(buf, "Trigger in progress for btn%d", btn); ////BUG, freeze!
//    server.send(200, httype, buf);
//    return 0;
//  }
  return btn;
